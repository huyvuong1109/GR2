# -*- coding: utf-8 -*-
"""
canonical_map.py - Map ten chi tieu OCR -> slug chuan theo tung loai cong ty.

Su dung:
    from canonical_map import map_to_canonical
    result = map_to_canonical("Doanh thu thuan ve ban hang", company_type="corporate")
    # -> ("KQKD", "doanh_thu_thuan")
"""
...
CORPORATE_MAP = {
    ...
    "vay_dai_han":                 ["vay va no thue tai chinh dai han", "vay dai han", "long term borrowings"],
    ...
    "tong_no_phai_tra":            ["tong no phai tra", "total liabilities", "tong cong no phai tra"],
    ...
    "cac_khoan_giam_tru_doanh_thu": ["cac khoan giam tru doanh thu", "deductions from revenue"],
    "doanh_thu_thuan": [
        "doanh thu thuan", "net revenue", "net sales",
        "doanh thu thuan ve ban hang va cung cap dich vu",
    ],
    "gia_von_hang_ban":            ["gia von hang ban", "cost of goods sold", "cost of sales"],
    "loi_nhuan_gop":               ["loi nhuan gop ve ban hang", "gross profit", "loi nhuan gop"],
    ...
    "chi_phi_ban_hang":            ["chi phi ban hang", "selling expenses"],
    "chi_phi_quan_ly_dn":          ["chi phi quan ly doanh nghiep", "general and administrative"],
    ...
    "loi_nhuan_truoc_thue": [
        ...
        "loi nhuan truoc thue", "earnings before tax",
    ],
    "chi_phi_thue_tndn":           ["chi phi thue thu nhap doanh nghiep", "income tax expense"],
    "loi_nhuan_sau_thue":          ["loi nhuan sau thue thu nhap doanh nghiep", "profit after tax", "net profit"],
    ...
    # LCTT
    "lctt_truoc_thay_doi_von_luu_dong": ["luu chuyen tien thuan tu hoat dong kinh doanh truoc"],
    ...
    "tien_mua_tai_san_co_dinh":    ["tien chi de mua sam tai san co dinh", "purchase of fixed assets"],
...
BANK_MAP = {
    ...
    "cac_khoan_no_chinh_phu_nhnn": ["cac khoan no chinh phu va ngan hang nha nuoc", "borrowings from government sbv"],
    "tien_gui_vay_cac_tctd":       ["tien gui va vay cac to chuc tin dung", "due to banks"],
    "tien_gui_khach_hang": [
        "tien gui cua khach hang", "customer deposits",
        "nhan tien gui cua khach hang", "tien gui",
    ],
    ...
    "loi_nhuan_chua_phan_phoi": [
        "loi nhuan chua phan phoi", "retained earnings", "loi nhuan sau thue chua phan phoi",
    ],
    ...
    "lai_thuan_tu_dich_vu":        ["lai thuan tu hoat dong dich vu", "net fee income", "phi dich vu thuan"],
    "lai_thuan_ngoai_hoi": [
...
SECURITIES_MAP = {
    ...
    "fvtpl": [
        "chung khoan fvtpl", "chung khoan ghi nhan theo gia tri hop ly qua lai suat",
...
# ==============================================================================
# KEYWORD INDEX - build 1 lan khi import
# ==============================================================================
...
# {company_type -> {norm_keyword -> slug}}
...
def map_to_canonical(
    ...
    Args:
        item_name   : ten chi tieu lay tu PDF (co the co loi OCR)
    ...
    # 3. Keyword chua item_name (OCR bi cat ngan - chi ap dung khi du dai)
    if len(n) >= 12:
        for kw, slug in idx.items():
            if n in kw:
                return slug
...
def get_statement_for_slug(slug: str, company_type: str = "corporate") -> str | None:
    ...
    Can thiet de ghi dung vao report_items.statement.
    """
